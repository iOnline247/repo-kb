...
llm-personal-kb/
|-- .claude/
|   |-- settings.json                # Hook configuration (auto-activates in GHCP/Claude Code)
...
|   |-- qa/                          #   Filed query answers (compounding knowledge)
|-- scripts/                         # CLI tools
|   |-- compile.ts                   #   Compile daily logs -> knowledge articles
|   |-- query.ts                     #   Ask questions (index-guided, no RAG)
|   |-- lint.ts                      #   7 health checks
|   |-- flush.ts                     #   Extract memories from conversations (background)
|   |-- config.ts                    #   Path constants
|   |-- utils.ts                     #   Shared helpers
|-- hooks/                           # Agentic Harness hooks
|   |-- session-start.ts             #   Injects knowledge into every session
|   |-- session-end.ts               #   Extracts conversation -> daily log
|   |-- pre-compact.ts               #   Safety net: captures context before compaction
...
Hooks are configured in `.claude/settings.json` and fire automatically when you use GHCP/Claude Code in this project.
...
{
  "hooks": {
    "SessionStart": [{ "matcher": "", "hooks": [{ "type": "command", "command": "npx --no-install tsx hooks/session-start.ts", "timeout": 15 }] }],
    "PreCompact": [{ "matcher": "", "hooks": [{ "type": "command", "command": "npx --no-install tsx hooks/pre-compact.ts", "timeout": 10 }] }],
    "SessionEnd": [{ "matcher": "", "hooks": [{ "type": "command", "command": "npx --no-install tsx hooks/session-end.ts", "timeout": 10 }] }]
  }
}
```
...
- The Agentic Harness sees the knowledge base index at the start of every session
- Max context: 20,000 characters

**`session-end.ts`** (SessionEnd)
...
- Spawns `flush.ts` as a detached background process
- Recursion guard: exits immediately if `COPILOT_INVOKED_BY` env var is set

**`pre-compact.ts`** (PreCompact)

- Same architecture as session-end.ts
- Fires before GHCP/Claude Code auto-compacts the context window
...
### Background Flush Process (`flush.ts`)
...
This ensures flush.ts survives after Claude Code's hook process exits.
...
const content = msg.content ?? ""; // string or list of content blocks
```
...
### compile.ts - The Compiler

Uses the GitHub Copilot SDK session API:

```ts
const session = await client.createSession({
  onPermissionRequest: approveAll,
  workingDirectory: ROOT_DIR,
  availableTools: ["read", "write", "edit", "glob", "grep"],
});
const response = await session.sendAndWait({ prompt: compilePrompt });
```
...
- `approveAll` auto-approves required file operations during compile/query runs
...
```bash
npm run compile --                            # compile new/changed only
npx --no-install tsx scripts/compile.ts --all
npx --no-install tsx scripts/compile.ts --file daily/2026-04-01.md
npx --no-install tsx scripts/compile.ts --dry-run
```

### query.ts - Index-Guided Retrieval
...
```bash
npm run query -- "What auth patterns do I use?"
npm run query -- "What's my error handling strategy?" --file-back
```
...
```bash
npm run lint:kb                                  # all checks
npm run lint:kb:structural                       # skip LLM check (free)
```
...
- `@github/copilot-sdk` - GitHub Copilot SDK for LLM calls and session control
- `tsx` - TypeScript runtime for hooks and scripts without a build step
- `typescript` and `@types/node` - Type-checking and Node typings

`package.json` (at project root):

- `tsx` - Runs TypeScript Claude hooks without a build step
- `typescript` and `@types/node` - Type checking and Node typings for hooks
...
Add directories like `people/`, `projects/`, `tools/` to `knowledge/`. Define the article format in this file (AGENTS.md) and update `utils.ts`'s `listWikiArticles()` to include them.
